...
$data=trim($data);
echo $data;
$serverName = "SERVTIENDA"; //serverName/instanceName, portNumber (por defecto es 1433)
$connectionInfo = array("Database"=>"APPBDMANTIS","UID"=>"sa", "PWD"=>"Mantis321");
...
foreach($manage->pedido as $obj){
	...
	//if($existe==0){     
		if($existe==0){
			...
			if ( $rs )
			{
				...
				if($vuelta==1){
					...
					VALUES('P',$secuencia,'PED',1,'$pdyear-$pdmonth-$pdday','miobs','admin',$plazodias,$registros,$listaprecio,$vencod,$nitsec,$clisec,'$id_pedido',$Total,7,2,NULL,NULL,NULL,'A')";					
					
					//$sql2="INSERT INTO Cotizaciones1(CotTip,CotSec,TipCod,EmpCod,CotFecha,CotObs,CotUsuCod,CotCliConPag,CotSecConCon,CotLisPreCod,CotSubVenCod,CotSubNitSec,CotSubCliSec,CotNum,CotSumCot,BodSucCCSec,CotEst,CotSubCotSec,AnuFueSec,CotAnuObs,CotEstado)
					//VALUES('P',$secuencia,'PED',1,'$pdyear-$pdmonth-$pdday','miobs','admin',$plazodias,$registros,$listaprecio,$vencod,$nitsec,$clisec,'$id_pedido',$Total,1,2,NULL,NULL,NULL,'A')";					
					echo $sql2;					
					...
					$sql3="insert into CotizacionesDetalle1(CotTip,CotSec,CotSecCon,CotObsequio,ArtSec,CotArtEmb,CotArtLot,CotArtLotFec,CotArtCaj,
					CotArtUni,CotArtDesUno,CotArtDesDos,CotArtDesTre,CotArtDesCua,CotArtDesVal,CotArtConIva,CotArtPrecio,CotSumDes,CotArtSubTotDesUno,
					CotArtSubTotDesDos,CotArtSubTotDesTre,CotArtSubTotDesCua,CotSubLisPreCod,
					...
					,(select top 1 ParConIva from Articulos a left join ParametroContable p on a.ParConCod=p.ParConCod where ArtSec=$artsec),NULL)";
		...
		//			$precio,0.00000,0.00000,0.00000,0.00000,0.00000,isnull((select lisprecod from clientes where nitsec=$nitsec and clisec=$clisec),0),(select top 1 PreArtCod from ArtPre where artsec=$artsec),$bodega,0,0,'A',0,0.00000,1.00000,NULL,0.00000,(select top 1 ParConIva from Articulos a left join ParametroContable p on a.ParConCod=p.ParConCod where ArtSec=$artsec),NULL)";
					...
					$precio,0.00000,0.00000,0.00000,0.00000,0.00000,isnull((select lisprecod from clientes where nitsec=$nitsec and clisec=$clisec),0)
					,(select top 1 PreArtCod from ArtPre where artsec=$artsec),$bodega,0,0,'A',0,0.00000,1.00000,NULL,0.00000
					,(select top 1 ParConIva from Articulos a left join ParametroContable p on a.ParConCod=p.ParConCod where ArtSec=$artsec),NULL,$cant*$precio)";
